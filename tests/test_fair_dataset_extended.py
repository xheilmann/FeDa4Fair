...
class TestFairDatasetExtended(unittest.TestCase):
    def setUp(self):
        self.df = pd.DataFrame({"feature1": [1, 2, 3, 4], "SEX": [1, 2, 1, 2], "label": [0, 1, 0, 1]})
    ...
    def test_modification_application(self, mock_load):
        ...
        mod_dict = {"train": {"SEX": {"drop_rate": 1.0, "value": 1}}}

        fds = FairFederatedDataset(
            dataset="generic", partitioners={"train": 1}, label_name="label", modification_dict=mod_dict
        )
        fds.prepare()
    ...
    def test_split_into_train_val_test(self, mock_divider, mock_load_acs):
        ...
        mock_divider_instance.return_value = DatasetDict({"CA_train": self.ds, "CA_val": self.ds, "CA_test": self.ds})
    ...
    def test_prepare_acs_dataset(self, mock_load_acs):
        ...
        fds = FairFederatedDataset(
            dataset="ACSIncome", states=["CA"], partitioners={"CA": 1}, label_name="label", sensitive_attributes=["SEX"]
        )
        fds.prepare()
