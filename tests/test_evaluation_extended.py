...
class TestEvaluationExtended(unittest.TestCase):
    def test_evaluate_fairness(self):
        ...
        mock_fig = MagicMock()
        mock_ax = MagicMock()
        df = pd.DataFrame({"class0": [1, 2]}, index=[0, 1])  # type: ignore[arg-type]
        ...
        with (
            patch.object(
                eval_mod, "plot_comparison_label_distribution", return_value=(mock_fig, [mock_ax], [df])
            ) as mock_pcld,
            patch.object(
                eval_mod, "plot_comparison_fairness_distribution", return_value=(mock_fig, [mock_ax], [df])
            ) as mock_pcfd,
            patch.object(eval_mod, "merge_dataframes_with_names", return_value=pd.DataFrame()),
            patch.object(eval_mod, "Path"),
            patch.object(eval_mod, "pickle"),
        ):
            evaluate_fairness(partitioner_dict=partitioner_dict, sens_columns=["SEX"])
    ...
    def test_evaluate_models_on_datasets(self, mock_show, mock_evaluate_model):
        ...
        # _get_models returns LogisticRegression + optionally XGBoost
        from FeDa4Fair.metrics.evaluation import XGBOOST_AVAILABLE
