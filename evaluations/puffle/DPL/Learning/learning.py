...
class Learning:
    ...
    def train_private_model(
        ...
        with BatchMemoryManager(
            data_loader=train_loader, max_physical_batch_size=512, optimizer=optimizer,
            ...
            for batch_number, (batch_data, sens_1, sens_2, _, batch_target) in enumerate(memory_safe_loader, 0):
                ...
                reg_results = Learning._handle_regularization(
                    train_parameters, model_regularization, data, target, sens_feature,
                    criterion_reg, average_probabilities, epoch, batch_number, node_id, wandb_run
                )
                ...
                classic_loss = Learning._compute_classic_loss(outputs, target, sens_feature, train_parameters, reweighing_weights)
                loss = Learning._handle_backward_and_gradients(train_parameters, reg_term, classic_loss, model, model_regularization, optimizer)
                ...
                if train_parameters.regularization_mode == "tunable" and fairness_violation is not None and train_parameters.target:
                    velocity = Learning._update_lambda_tunable(
                        train_parameters, model, data, target, sens_feature,
                        criterion_reg, average_probabilities, epoch, batch_number,
                        sigma_update_lambda, velocity
                    )
        ...
        max_unfairness_train = criterion_reg.violation_with_dataset(
            model=model, dataset=train_loader, device=train_parameters.device,
            average_probabilities=average_probabilities,
        )
        return {
            "epoch": current_epoch, "Train Loss": np.mean(losses),
            "Train Loss + Regularizaion": np.mean(losses_with_reg),
            "Train Accuracy": total_correct / total,
            "Max Unfairness Train": max_unfairness_train, "history_lambda": history_lambda,
        }
    ...
    def _handle_regularization(
        train_parameters, model_regularization, data, target, sensitive_feature,
        criterion_regularization, average_probabilities, epoch, batch_number, node_id, wandb_run
    ):
        ...
        if wandb_run:
            val = regularization_term.item() if isinstance(regularization_term, torch.Tensor) else (regularization_term or 0)
            wandb_run.log({
                "batch": (epoch + 1) * batch_number,
    ...
    @staticmethod
    def _handle_backward_and_gradients(train_parameters, regularization_term, classic_loss, model, model_regularization, optimizer):
        loss = (1 - train_parameters.regularization_lambda) * classic_loss if regularization_term is not None else classic_loss
        loss.backward()
    ...
    def _update_lambda_tunable(
        train_parameters, model, data, target, sensitive_feature,
        criterion_regularization, average_probabilities, epoch, batch_number,
        sigma_update_lambda, velocity
    ):
        model.eval()
        with torch.no_grad():
        ...
        train_parameters.regularization_lambda = np.clip(train_parameters.regularization_lambda - train_parameters.alpha * velocity, 0, 1)
        return velocity
    ...
    def test(
        ...
        return Learning._test_base(
            model, test_loader, train_parameters, average_probabilities,
            sensitive_attr_index=1, violation_method="violation_with_dataset"
        )
    ...
    def test_2(
        ...
        res = Learning._test_base(
            model, test_loader, train_parameters, average_probabilities,
            sensitive_attr_index=2, violation_method="violation_with_dataset_2"
        )
    ...
    def test_3(
        ...
        return Learning._test_base(
            model, test_loader, train_parameters, average_probabilities,
            sensitive_attr_index=3, violation_method="violation_with_dataset_3"
        )
    ...
    def _test_base(
        ...
        with torch.no_grad():
            for batch in test_loader:
                batch_data, batch_target = batch[0].to(train_parameters.device), batch[4].long().to(train_parameters.device)
        ...
        if train_parameters.metric == "disparity":
            ...
            unfairness_test, max_group = method(
                model=model, dataset=test_loader, device=train_parameters.device,
                average_probabilities=average_probabilities, return_group=True,
            )
        y_true_list, y_pred_list = [item.item() for item in y_true], [item.item() for item in y_pred]
        return (
            np.mean(losses), correct / total, f1_score(y_true_list, y_pred_list, average="macro"),
            precision_score(y_true_list, y_pred_list, average="macro"),
            recall_score(y_true_list, y_pred_list, average="macro"),
            unfairness_test, y_true_list, y_pred_list, colors, max_group
        )
    ...
    def compute_regularization_term(
        ...
        return criterion_regularization(
            sensitive_attribute_list=sensitive_feature, device=train_parameters.device,
            predictions=outputs, possible_sensitive_attributes=list({item.item() for item in sensitive_feature}),
            possible_targets=list({item.item() for item in targets}),
            average_probabilities=average_probabilities, wandb_run=wandb_run, batch=batch,
        )
    ...
    def test_prediction(
        ...
        return (
            torch.cat(predictions, dim=0), torch.tensor(sensitive_attributes),
            torch.tensor(second_sensitive_attributes), torch.tensor(third_sensitive_attributes),
            set(y_true_items), set(sensitive_attributes), set(second_sensitive_attributes),
            set(third_sensitive_attributes), y_true_items,
        )
