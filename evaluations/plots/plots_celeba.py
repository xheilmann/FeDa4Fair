...
def plot_attribute_unfairness(csv_path, output_dir, file_prefix, title_suffix=""):
    ...
    # Rename for display
    df_plot = df.rename(columns={"Male_DP": "Male", "hair_color_DP": "Hair Color", "Partition ID": "Client ID"})
    ...
    sns.barplot(x="Client ID", y="Unfairness Score", hue="Attribute", data=df_melted, palette=custom_palette, ax=ax)
    ...
    handles = [mpatches.Patch(color=COLOR_MALE, label="Male"), mpatches.Patch(color=COLOR_HAIR, label="Hair Color")]
...
def plot_attribute_scatter(csv_path, output_dir, file_prefix, title_suffix=""):
    ...
    ax.scatter(
        df[x_col], df[y_col], facecolors=COLOR_DEFAULT, edgecolors="black", marker="o", s=200, linewidths=1.2, alpha=0.8
    )
...
def plot_value_lollipop_sorted(csv_path, output_dir, file_prefix, title_suffix=""):
    ...
    plot_df = pd.DataFrame({"Client ID": df["Partition ID"], "Demographic Parity": max_dp, "Value": values})
...
def plot_value_boxplot_grouped(csv_path, output_dir, file_prefix, title_suffix=""):
    ...
    plot_df = pd.DataFrame({"Demographic Parity": max_dp, "Value": values})
...
def plot_value_distribution_scatter(csv_path, output_dir, file_prefix, title_suffix=""):
    ...
    for col in df.columns:
        if "hair_color" not in col:
            continue
        if col in ["hair_color_DP", "hair_color_val", "Max_DP"]:
            continue
        val = -1
        if "_0_" in col or col.endswith("_0"):
            val = 0
        elif "_1_" in col or col.endswith("_1"):
            val = 1
        elif "_2_" in col or col.endswith("_2"):
            val = 2
        elif "_3_" in col or col.endswith("_3"):
            val = 3
