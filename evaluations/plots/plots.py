...
def bar_plot_differences(
    ...
    save: bool = False,
    fig_path: str = "",
    legend_name: str = "bar_plot_differences_legend",
):
    """
    ...
    df_melted = df_plot.melt(
        id_vars="State", value_vars=["DP_SEX", "DP_RACE"], var_name="Sensitive Attribute", value_name="Unfairness Score"
    )
    ...
    sns.barplot(
        x="State", y="Unfairness Score", hue="Sensitive Attribute", data=df_melted, palette=custom_palette, ax=ax
    )
    ...
    if legend:
        ...
        if save:
            ...
            ax_legend.legend(handles, labels_legend, loc="center", fontsize=font_size_ticks, ncol=2, frameon=False)
            ax_legend.axis("off")
...
def local_client_fairness_plot(
    ...
    ax.plot([diag_min, diag_max], [diag_min, diag_max], linestyle="dotted", color="gray", linewidth=2)
    ...
    if file_path.endswith(".json"):
        ...
        if isinstance(data, dict) and len(data) > 0:
            ...
            if isinstance(first_val, dict) and (
                any(k in first_val for k in ["lr", "xgb", "rf", "svm", "LogisticRegression", "XGBoost"])
                or "accuracy" not in first_val
            ):
                rows = []
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="If set, removes suffixes like '_0' from dataset names in FL results (useful for Cross-Silo)",
    )
    parser.add_argument(
        "--num_clients", type=int, default=50, help="Number of clients for Dutch dataset distribution logic"
    )
    parser.add_argument(
        "--experiment_type",
        default="baseline",
        choices=["baseline", "puffle", "reweighing"],
        help="Type of experiment (baseline, puffle, reweighing)",
    )
    ...
    if args.states_unfairness_path:
        ...
        legend_labels = {"race": "Race-Related States", "sex": "Sex-Related States"}
    ...
    for model in models:
        ...
        try:
            ...
            # Check columns
            cols_to_check = []
            if "DP_SEX" in diff_df.columns:
                cols_to_check.append("DP_SEX")
            if "DP_RACE" in diff_df.columns:
                cols_to_check.append("DP_RACE")
        ...
        if (
            "DP_SEX" in model_df.columns
            and "DP_RACE" in fl_df.columns
            and "DP_SEX" in fl_df.columns
            and "DP_RACE" in model_df.columns
        ):
            try:
